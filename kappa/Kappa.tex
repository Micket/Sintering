...
However, in the case when pores and surface tension are present, as in \refpaper{D}, it is necessary to choose the prolongation in a non-standard fashion
\begin{align}
 \ta v^\macro = \bar{\ta v} + [\bar{\ta v}\outerp\diff]\cdot[\ta x - \bar{\ta x}]
\label{eq:vm_d}
\\
...
\label{eq:avg_p_d}
\end{align}
With these choices, we maintain the macroscale problem as \cref{eq:prototype_macro}, as shown in detail in \refpaper{D}.
...
\label{eq:rve_b}
\\
%  -\frac{1}{\volume} \int_{\Gamma_\rve} \dif &= -\bar{p} &&\text{ in } \Omega_\rve^\particle
% \label{eq:rve_b}
% \\
...
The Neumann and weakly periodic boundary conditions for the RVE-problem is also possible, and are shown in \refpaper{D}.

As an alternative, the procedure in \refpaper{B} pure algebraic manipulation was used to rewrite \cref{eq:prototype_macro_A} to enable the simulations to transition into the macroscopically incompressible limit.
