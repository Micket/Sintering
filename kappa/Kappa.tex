...
\begin{tikzpicture}
 ...
 \draw[latex-] (1.35,1.5) -- +(-0.5,0) node[left] {$\Gamma_\rve$};
 \node at (2.5,1.5) {RVE};
...
A general review of VCH can be found in Appendix A in \refpaper{C}.
...
In \refpaper{A}, the chosen split was defined by the first order Taylor expansion of the velocity field, and no macroscale pressure, i.e.
\begin{align}
...
With \refpaper{B}, purely algebraic manipulation was used to rewrite the system of equations and doing so leads to exactly the problem in \cref{eq:prototype_macro}.
...
In \refpaper{C}, first order Taylor expansion of the velocity (or displacement) field, and zeroth order expansion for the pressure field
\begin{align}
 \ta v^\macro &= \bar{\ta v} + [\bar{\ta v}\outerp\diff]\cdot[\ta x - \bar{\ta x}]
\\
 p^\macro &= \bar{p}
...
However, in the case when pores and surface tension are present, as in \refpaper{D}, it is necessary to choose the prolongation in such non-standard fashion
\begin{align}
 \ta v^\macro = \bar{\ta v} + [\bar{\ta v}\outerp\diff]\cdot[\ta x - \bar{\ta x}]
\label{eq:vm_d}
\\
...
\label{eq:avg_p_d}
\end{align}
With these choices, we maintain the macroscale problem as \cref{eq:prototype_macro}, as shown in the derivations in \refpaper{D}.
...
% \end{align}
% 
% With the choices in \cref{eq:vm_d,eq:pm_d} and \cref{eq:avg_v_d,eq:avg_p_d}, as in \refpaper{D}, VCH leads to the expected macroscale problem as stated in \eqref{eq:prototype_macro}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Numerical aspects}

In \refpaper{B}--\refpaper{C} the macroscale problems, as shown in \cref{eq:prototype_macro}, will also require stabilization and the Taylor-Hood approximation is the obvious choice.
...
\begin{itemize}
 ...
 \item To deal with large topological changes in the RVE-problems, a surface tracking algorithm capable of tracking multiple phases, allowing for vanishing pores, and tracking multiple regions for boundary conditions, was implemented for \refpaper{A}. 
   This allows for the Lagrangian mesh to follow the free surface flow of the sintered particles to the point of vanishing porosity, as the FE\textsuperscript{2}-simulations in \refpaper{B} demonstrates.
