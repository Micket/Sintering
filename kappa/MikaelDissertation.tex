...
2D RVE’s are used predominantly; however, 3D-mesostructures are also analyzed. Within the FE\textsuperscript{2} algorithmic setting, the homogenization is carried out at the Gaussian integration points in the macroscale FE-mesh.
This allow for the investigation of properties that are not easily captured with traditional macroscopic constitutive models, which inevitably would become highly complex with many material parameters that lack physical interpretation.
...
\paperwork{
The appended papers were prepared in collaboration with the co-authors.
The author of this thesis was responsible for the major progress of the work in preparing the papers, i.e. took part in planning the papers, took part in developing the theory, developed the numerical implementation and carried out the numerical simulations.
}
...
\begin{itemize}
 ...
 \item[$\bullet_\dev$] Subscript d represents deviatoric part of a 2nd order tensor
 \item[$\bar{\bullet}$] Bar represents macroscopic variables
 \item[$\bullet^\macro$] Superscript M represents the local prolongation of the macroscopic fields
 \item[$\bullet^\fluct$] Superscript s represents fluctuation fields
 \item[$\hat{\bullet}$] Hat denotes a constitutive law
 \item[$f\{\bullet\}$] Curly brackets are used for implicit functions
 \item[$\homgen{\bullet}$] Volume average over particles
 \item[$\shomgen{\bullet}$] Volume average over pore surface
 %\item[$\ta u$] Displacement
 \item[$\ta v$] Velocity
 \item[$p$] Pressure
 \item[$e$] Volumetric rate of deformation ($\ts d\dprod\ts I$)
 %\item[$\ts\epsilon$] The symmetric part of the deformation gradient ($[\ta u\outerp\diff]^\sym$)
 \item[$\ts d$] The symmetric part of the rate of deformation gradient ($[\ta v\outerp\diff]^\sym$)
 \item[$\ts\sigma$] Cauchy stress
 \item[$\bar{\tf E}$] $\displaystyle\defeq\pd{\bar{\ts\sigma}}{\bar{\ts d}}$
 \item[$\bar{\tf E}_\ded$] $\displaystyle\defeq\pd{\bar{\ts\sigma}_\dev}{\bar{\ts d}_\dev}$
 \item[$\bar{\ts E}_\dep$] $\displaystyle\defeq\pd{\bar{\ts\sigma}_\dev}{\bar{p}}$
 \item[$\bar{\ts C}_\ded$] $\displaystyle\defeq\pd{\bar{e}}{\bar{\ts d}_\dev}$
 \item[$\bar{C}_\dep$] $\displaystyle\defeq-\pd{\bar{e}}{\bar{p}}$
\end{itemize}
} % Optional
