...
% % % 
% % %     \addplot[red]
% % % 	table[x=co_fracs_mean, y=cont_mean] {stat_vol_random5.0.txt};
% % %     \addlegendentry{Random placement}
% % % 
% % %     \addplot[green]
% % % 	table[x=co_fracs_mean, y=cont_mean] {stat_vol_preopt5.0.txt};
% % %     \addlegendentry{Repulsive forces}
% % %     
% % %     \addplot[blue]
% % % 	table[x=co_fracs_mean, y=cont_mean] {stat_vol_packed5.0.txt};
% % %     \addlegendentry{Random translations}
% % %   \end{axis}
% % %   \end{tikzpicture}
% % %   
% % % %   \includegraphics[width=0.6\linewidth]{Result_plots/runs1/contiguity}
% % %   \caption{Typical trends of the contiguity $C$ as function of Co volume fraction $v_\text{Co}$ for the different grain placement strategies compared with experimental values by Luyckx et al. (2006) and Exner (1979).}
...
% % % % % 	\end{subfigure}
% % % % 	\caption{A comparison of four different methods of grain placement and simulation.
% % % % 	The input parameters are chosen such that $L=\SI{5}{\micro\meter}$, $M=200$.
% % % % 	In the random translation method, $\Delta$ is restricted so that a grain is moved maximally $\SI{0.75}{\micro\meter}$ in any direction and 2500 translations are tested for each grain.
% % % % 	The Potts simulation is made with $5000 \cdot M^3$ MC iterations with $\tau = 0.5$.
% % % % 	All results shown are taken as an average of 16 realizations.
% % % % 	The intervals marks two standard deviations.}
% % % % 	\label{fig:runs1}
% % % \end{figure}
% % % 
% % % 
% % % The contiguity measure $C$ is directly related to the grain packing, and has been our main goal to capture.
% % % 
% % % The motivation for adopting the strategy of the random translations were due to the fact that the packing obtain was insufficient, in Figure~\ref{fig:runs1_contiguity}.
% % % It must be noted that the this grain packing is intended to simulate real engineering materials where $v_\Co \leq 0.3$.
% % % 
% % % In order to achieve sufficient packing, $\Delta$ needs to be set so high that the initial positions have no influence on the packing result.
% % % The conclusion was therefor to fix the parameter $\Delta = L/2$ and drop the optimization using the repulsive potential for the results presented in this paper.
% % % The functionality is left in CCBuilder for possible future use as results can differ depending on many factors, such as volume fraction, grain shape and size distributions.
...
%     \addlegendentry{Coarse}
    ...
    \addplot[gray] table[x index=0,y index=1]  {exner_table.txt};
    \addlegendentry{Exner (1979)}
