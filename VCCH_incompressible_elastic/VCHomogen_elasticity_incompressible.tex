...
The macro homogeneity condition requires that $R(z, (z^\fluct)'\{z^\macro;\delta z^\macro\}) = 0$, which in this problem will be expanded to
...
\end{align}
\end{subequations}
The tangent problem at equilibrium with a perturbation of the macroscopic fields $\ta u^\macro$ and $p^\macro$
\begin{subequations}
\begin{alignat}{3}
...
\end{alignat}
\end{subequations}
where the sensitivity field $(\ta u^\fluct)'\{\ta u^\macro, p^\macro;\delta \ta u^\macro, \delta p^\macro\}$ is defined as the solution to be the solution $\dif\ta u^\fluct$.
Upon combining \cref{eq:UP_subset} and \cref{eq:vanishing_traction} with \cref{eq31} we can see that \cref{eq:macro_homogeneity_original} is fulfilled.
