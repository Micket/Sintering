...
%    \homgen{ \hat{\ts{\sigma}}_\dev(\ts{\epsilon}_\dev[\tilde{\ta{u}}]) }
%\label{eq53a} \\
%    \tilde{\bar{\epsilon}}_\vol\{\bar{\ts\epsilon}_\dev,\bar{p}\} = \bar{e}\{\bar{\ts\epsilon}_\dev,\bar{p}\} &\defeq&
%    \homgen{ \hat{e}(\tilde{p}) }
%\label{eq53b}
%\end{eqnarray}
%%---------------------------------------------------------------------------------------------------------------------
...
\begin{multline}
  \bar{\ts\sigma}_\dev\{\bar{\ts\epsilon}_\dev,\bar{p}\} = a_\rve(\ta u; (\ta u)'_{\epsilon_\dev}) + b_\rve(p, (\ta u)'_{\epsilon_\dev}) + d_\rve(\ta t, (\ta u)_{\epsilon_\dev}') + b_\rve((p)_p', \ta u) + c_\rve^*(p; (p)_{\epsilon_\dev}') + 
\\ + \bar{e} + \bar{p}\,(\bar{e})_{\epsilon_\dev}' - d_\rve(\ta t, (\bar{e})_{\epsilon_\dev}'\ta x_\mean) + d_\rve((\ta t)_{\epsilon_\dev}', \ta u - \bar{\ts\epsilon}_\dev\cdot[\ta x - \bar{\ta x}] - \bar{e}\,\ta x_\mean) = \bar{e}
\end{multline}
and
\begin{multline}
  \bar{e}\{\bar{\ts\epsilon}_\dev,\bar{p}\} = a_\rve(\ta u; (\ta u)'_p) + b_\rve(p, (\ta u)'_p) + d_\rve(\ta t, (\ta u)_p') + b_\rve((p)_p', \ta u) + c_\rve^*(p; (p)_p') + 
\\ + \bar{e} + \bar{p}\,(\bar{e})_p' - d_\rve(\ta t, (\bar{e})_p'\ta x_\mean) + d_\rve((\ta t)_p', \ta u - \bar{\ts\epsilon}_\dev\cdot[\ta x - \bar{\ta x}] - \bar{e}\,\ta x_\mean) = \bar{e}
\end{multline}
where we have used the equilibrium state in \cref{eq:rveStat} by choosing the test functions correspondingly; e.g. $\delta\ta u = (\ta u)_{\epsilon_\dev}'$.
