...
The typical dimension of an SVE is $L_\Box=\volume^{1/3}$.
...
Here it is considered as a \emph{model assumption}; however, it is also possible to view this choice as a (crude) FE-approximation of the traction field. $\Box$
...
\todo{show?}
We need to show that these sensitivities truly are the derivatives. We do so for Dirichlet in paper 2!\textbf{??????}.


\subsubsection{Periodic boundary condition}
Using the perturbations $\bar{\ts\epsilon}_{\dev} + \dif\bar{\ts\epsilon}_{\dev}$ and $\bar{p} + \dif\bar{p}$ in the linearized form of \cref{eq:?????} at equilibrium we obtain
\begin{subequations}
\begin{alignat}{3}
...
\end{alignat}
\end{subequations}
which must hold for any given $\dif\bar{\ts\epsilon}_{\dev}$ and $\dif\bar{p}$. We thus consider the cases
\begin{enumerate}
...
\begin{align}
 \hat{\bar{\ts\sigma}}_{\dev,\ded}^{(k)} &=  \frac{1}{\volume} \int_{\Gamma_\Box} \hat{\ta t}_{\ded}^{(k)} \outerp \ta n\dif S\\
 \hat{\bar{\ts\sigma}}_{\dev,\dep} &= \frac{1}{\volume} \int_{\Gamma_\Box} \hat{\ta t}_{\dep} \outerp \ta n\dif S
\end{align}
\todo{check}
...
\end{alignat}
\end{subequations}
which must hold for any given $\dif\bar{\epsilon}_{\dev,i}$ and $\dif\bar{p}$. We thus consider the cases
\begin{enumerate}
