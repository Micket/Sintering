...
Among the seminal work on computational homogenization, we mention \textsc{Miehe et al. (1999)} \cite{Mieheetal1999}, 
%\textsc{Zohdi \& Wriggers (2005)} \cite{ZohdiWriggers2005},
\cite{Fillepetal2013a},
\cite{Larsson_etal2011},
\cite{Fish2006}, \cite{Fishetal1997},
\cite{Geersetal2009},\cite{Ozdemiretal2008},
\cite{Hazanovetal1994},
\cite{Klinge2012},
\cite{Lindfeldt???}
.
...
This process is thus characterized by a transition from the compressible to incompressible regimes that should be handled within the same variational framework, cf.\  \textsc{Öhman et al.\ (2013)} \cite{Ohman2013a}.
...
Since a potential exists, it follows that $\bar{\tf {E}}$ is symmetric and that $\bar{\ts E} = \bar{\ts C}$, cf.\ \"Ohman et. al \cite{Ohman2013a}.
...
\subsection{Bounds on the homogenized energy density}
...
on the homogenized energy density that is obtained with periodic boundary conditions for any given realization of the RVE and a given macroscale state $(\bar{\ts\epsilon}_\dev, \bar{p})$.
...
\end{align}
%---------------------------------------------------------------------------------------
Tangent operators associated with $\bar{e}$ are obtained directly from sensitivity analysis
\begin{align}
 \dif\bar{e} = \underbrace{\sum_k \bar{e}_\ded^{(k)} \ts G_k}_{\bar{\ts C}} \dprod \dif\bar{\ts\epsilon}_\dev + \underbrace{\bar{e}_\dep}_{\bar{C}} \dif\bar{p}
\end{align}
