...
In practice, the SVE's are finite-sized and occupies the subscale region $\Omega_\rve$ with boundary $\Gamma_\rve$.
...
%----------------------------------------------------------------------------
We can thus establish at the outset, before any further analysis, that the displacement and pressure fields within each SVE are implicit functions of the values $\bar{\ta u}$, $\bar{\ts h}$, $\bar{p}$, such that $\ta u = \tilde{\ta u}\{\bar{\ta u}, \bar{\ts h}, \bar{p}\}$ and $p = \tilde{p}\{\bar{\ta u}, \bar{\ts h}, \bar{p}\}$.
...
\end{align}
%----------------------------------------------------------------------------------------------------------------
By combining \cref{eq4} with \cref{eq12} we note that $\bar{\ts\sigma}_\dev$ and $\bar{e}$ are indeed implicit functions of the values of the macroscale variables $\bar{\ta u}$, $\bar{\ts h}$ and $\bar{p}$, pertinent to the considered SVE.
...
\end{equation}
%----------------------------------------------------------------------------------------------------------------------
We may deduce that $\bar{\psi}_\rve$ serves as the ``effective energy density'' for $\bar{\ts\sigma}_\dev$ and $\bar{e}$ in the sense that we have the macroscale constitutive relations
%----------------------------------------------------------------------------------------------------------------
\begin{equation}
...
\textbf{Remark}:
The VMCM in \cref{eq:macro_homogeneity_original} is still valid for the restricted space $\set{U}_\rve^{\Dirichlet,0}\subset \set{U}_\rve^\fluct$.
Since $d_\rve(\delta\ta t^\fluct, \ta u^\fluct) = 0\;\forall\,\ta u^\fluct \in \set U_\rve^{\Dirichlet,0}$, \cref{eq:macro_homogeneity_original_a} is satisfied for all $\delta\ta u^\fluct\in\set U^{\Dirichlet,0}$. $\Box$
...
The pertinent SVE-problem is obtained from the general format upon restricting the space $\set{T}_\rve$, i.e.\ introducing $\set{T}_\rve^\Neumann\subseteq\set{T}_\rve$ defined as
%----------------------------------------------------------------------------
\begin{equation}
    \set{T}_\rve^\Neumann =
    \{\ta{t}\in\set{T}_\rve\,|\,\, \exists(\check{\ts\sigma}_\dev,\check{p})\in\set{R}^{3\times3}_\dev \times\set{R} \text{ s.t. }
...
\begin{equation}
    ...
    \sup_{\substack{
    \hat{p}\in\set{P}_\rve \\
    \check{\ts\sigma}\in\set{R}^{3\times3}_\dev \\
    \check{p}\in\set{R}
    }}
    \inf_{
    \hat{\bar{e}}\in\set{R}
...
From \cref{eq:weak_periodic_sigma_dev} we can see that for the restricted tractions $\ta t \in \set T_\rve^\Neumann$ we obtain the macroscopic deviatoric stress as $\check{\ts\sigma}_\dev$, i.e.\ $\bar{\ts\sigma}_\dev\{\bar{\ts\epsilon}_\dev,\bar{p}\} \defeq
\homgen{ \hat{\ts{\sigma}}_\dev(\ts{\epsilon}_\dev[\ta{u}\{\bar{\ts\epsilon}_\dev,\bar{p}\}]) } = \check{\ts\sigma}_\dev$.
%----------------------------------------------------------------------------------------------------------------------
Hence, we replace the saddle-point problem by
%----------------------------------------------------------------------------------------------------------------
\begin{align}
...
%\nonumber \\
%    &=& \inf_{\tilde{\ta{u}}^\fluct\in\set{U}^{\Dirichlet,0}} \sup_{\tilde{p}\in\set{P} \tilde{\ta{t}}\in\set{T}} \inf_{\check{e}\in\set{R}}
%    \left[\Lambda_\rve(\tilde{\ta{u}},\tilde{p}) - \bar{p}\,\check{e}\right]
\label{eq98a}
\end{align}
%----------------------------------------------------------------------------------------------------------------------
...
\end{equation}
%----------------------------------------------------------------------------------------------------------------------
such that, typically, the residual in \cref{eq:A2a} can be rewritten in terms of the contributions defined on each SVE as
%--------------------------------------------------------------------------------------------------------------------------
\begin{align}
...
\end{align}
where the SVE-potential $\Pi_\rve$ was defined in \cref{eq81}.
...
\nonumber\\
  = \homgen{\ts\sigma_\dev\dprod\dif\bar{\ts\epsilon}_\dev} + \underbrace{\homgen{p\,\dif\bar{\ts\epsilon}_\dev\dprod \ts I}}_{=0} 
...
\begin{subequations}
\begin{alignat}{3}
    (a_\rve)'(\ta{u};\delta\ta{u}, \Delta\ta u) + b_\rve(\Delta p,\delta\ta{u}) +  d_\rve(\Delta\bar{\ts\sigma}_\dev\cdot\ta n,\delta\ta{u}) &= d_\rve(\dif\bar{p}\,\ta n, \delta\ta u)
    & \quad & \forall \delta\ta{u} &&\in \set{U}_\rve
\\
    b_\rve(\delta p,\Delta\ta{u}) + (c^*_\rve)'(p;\delta p, \Delta p) &= 0
    & \quad & \forall \delta p &&\in \set{P}_\rve
\\
    d_\rve(\delta\bar{\ts\sigma}_\dev\cdot\ta n,\Delta\ta{u}) &= - \dif\bar{\ts\epsilon}_\dev\dprod\delta\bar{\ts\sigma}_\dev
    & \quad & \forall \delta\bar{\ts\sigma}_\dev &&\in\set{R}^{3\times3}_\dev
\end{alignat}
\end{subequations}
which must hold for any given $\dif\bar{\epsilon}_{\dev,i}$ and $\dif\bar{p}$.
...
\begin{alignat}{3}
    (a_\rve)'(\ta{u};\delta\ta{u}, \ta u_\dev^{(k)}) + b_\rve(p_\dev^{(k)},\delta\ta{u}) +  d_\rve(\bar{\sigma}_{\dev,i,\dev}^{(k)}\ts E_i\cdot\ta n,\delta\ta{u}) &= 0
    & \quad & \forall \delta\ta{u} &&\in \set{U}_\rve
\\
    b_\rve(\delta p,\ta u_\dev^{(k)}) + (c^*_\rve)'(p;\delta p, p_\dev^{(k)}) &= 0
    & \quad & \forall \delta p &&\in \set{P}_\rve
\\
    d_\rve(\delta\bar{\sigma}_{\dev,i} \ts E_i\cdot\ta n,\ta u_\dev^{(k)}) &= - \delta_{ik}\;\delta\bar{\sigma}_{\dev,i}
