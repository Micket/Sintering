...
%\begin{align}
%    \Pi(\ta{u},p) &\defeq
%   \int_{\Omega} \left[\psi_u(\ts{\epsilon}_\dev[\ta{u}]) - p\,e[\ta{u}] + \psi_p^*(p)\right] \dif V
...
\end{subequations}
%------------------------------------------------------------------------------------------------------------
where curly brackets indicate an implicit function.
...
%----------------------------------------------------------------------------
\begin{align}
    \frac{1}{\volume} \int_{\Gamma_\rve} \ta{u}^\fluct \dif V &= \ta{0}, \quad
...
%----------------------------------------------------------------------------
\begin{align}
    \bar{\ta{u}} &=\frac{1}{\volume} \int_{\Gamma_\rve} \ta{u} \dif V, \quad
...
\end{align}
%----------------------------------------------------------------------------
We can now establish that the displacement and pressure fields within each SVE are implicit functions of the values $\bar{\ta u}$, $\bar{\ts h}$, $\bar{p}$, such that 
\begin{subequations}
\begin{align}
...
\label{eq15e} \\
    ...
    \frac{1}{\volume}\int_{\Gamma_\rve}  \delta\bar{\ta{u}} \cdot \bar{\ta t}_\prescribed \dif S =
    \delta\bar{\ta{u}} \cdot \bar{\ta t}_\prescribed
\label{eq15f}
\end{align}
%---------------------------------------------------------------------------------------------------------------------
whereby the variationally consistent macroscale flux variables are obtained from homogenization of the constitutive functions as follows:
%----------------------------------------------------------------------------------------------------------------
...
\begin{align}
    ...
    - \frac{1}{\volume}\int_{\Omega_\rve} p\,e[\delta\ta{u}] \dif V
    - \frac{1}{\volume}\int_{\Gamma_\rve} \ta{t}\cdot\delta\ta{u} \dif S &= 0
\label{eq20a} \\
    - \frac{1}{\volume}\int_{\Omega_\rve} \delta p\,e[\ta{u}] \dif V
...
We are now in the position to establish the subscale problem: For \emph{given} values $\bar{\ta{u}}$, $\bar{\ts{h}}$, and $\bar{p}$, that represent the macroscale fields (and which solve the macroscale problem), we first make the unique decomposition:
%----------------------------------------------------------------------------
...
\label{eq27b}
\end{equation}
hence, \cref{eq27a} can be restricted to test functions in $\Pi_\rve^\fluct$, such that the stress quantity $\bar{\ts\tau}$ is not part of the RVE-problem.
...
%    \quad \forall \delta\ta{u}^\fluct \in \set{U}_\rve^\fluct
%\label{eq31a} \\
%    - \frac{1}{\volume}\int_{\Omega_\rve} \delta p^\flucte[\ta{u}^\fluct] \dif V
...
\end{align}
%---------------------------------------------------------------------------------------------------------------------
Note that $\bar{e}\{\bar{\ts\epsilon}_\dev,\bar{p}\}\neq\bar{h}_\vol$ in general! In fact, in addition to the conventional situation that $\bar{\ta u}$ and $\bar{\ts h}^\skw$ do not affect the subscale solution, $\bar{h}_\vol$ does not affect the solution either.
...
\begin{align}
 \homgen{\hat{e}(p)} = b_\rve(1, \ta u) = d_\rve(\ta n, \ta u) = d_\rve(\ta n, \bar{e}\,\ta x_\mean) = d_\rve(\ta n, \ta x_\mean)\bar{e} = b_\rve(1, \ta x_\mean)\bar{e} = \bar{e}
\end{align}
where the divergence theorem was used in the steps $b_\rve(1, \bullet) = d_\rve(\ta n, \bullet)$.
...
% \end{subequations}
%---------------------------------------------------------------------------------------------------------------------
The solution to this ``canonical'' problem contains, in fact, the solution to the original ``variationally consistent'' problem in \cref{eq31}; however, without the apparent drawback that is associated with that format. The fluctuations
%----------------------------------------------------------------------------
\begin{subequations}
\begin{align}
...
\textbf{Discuss VCMC here??? 4.2 or 4.3 Fredrik??}\todo{todo}
...
\begin{subequations}\label{eq67}
\begin{alignat}{3}
    a_\rve(\ta{u},\delta\ta{u}) + b_\rve(p,\delta\ta{u}) +  d'_\rve(\tilde{\bar{\ts\sigma}}_\dev,\delta\ta{u}) &= - \bar{p}\,\homgen{e[\delta\ta{u}] }
    & \quad & \forall \delta\ta{u} &&\in \set{U}_\rve
\label{eq67a} \\
    b_\rve(\delta p,\ta{u}) + c^*_\rve(p;\delta p) &= 0
    & \quad & \forall \delta p &&\in \set{P}_\rve
