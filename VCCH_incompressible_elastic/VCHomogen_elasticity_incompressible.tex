...
\begin{align}
 \hat{\bar{\ts\sigma}}_{\dev,\ded}^{(k)} &=  \frac{1}{\volume} \int_{\Gamma_\rve} \hat{\ta t}_{\ded}^{(k)} \outerp [\ta x - \bar{\ta x}]\dif S + \bar{p}\ts I\\
 \hat{\bar{\ts\sigma}}_{\dev,\dep} &= \frac{1}{\volume} \int_{\Gamma_\rve} \hat{\ta t}_{\dep} \outerp [\ta x - \bar{\ta x}]\dif S + \bar{p}\ts I
\end{align}
...
Upon using the identity $\bar{\ts\sigma}=\frac{1}{\volume} \int_{\Gamma_\Box} \ts t \outerp [\ta x - \bar{\ta x}]\dif S$, we deduce the component representation
%---------------------------------------------------------------------------------------
\begin{align}
    ...
    = (a_\Box)'(\ta{u}; \hat{\ta{u}}^{\macro(i)}_\dev,\dif\ta{u})
\nonumber \\
    & =
    \sum_j \underbrace{(a_\Box)'(\ta{u};\hat{\ta{u}}^{\macro(i)}_\dev,\hat{\ta{u}}^{\macro(j)}_\dev +
    \hat{\ta{u}}^{\fluct(j)}_\ded)}_{\displaystyle \hat{\bar{\sigma}}_{\dev,i,\ded}^{(j)}}
    \dif\bar{d}_{\dev,j}
% Second term
    + \underbrace{(a_\Box)'(\ta{u};\hat{\ta{u}}^{\macro(i)}_\dev, \hat{\ta{u}}^{\fluct}_\dep)}_{\displaystyle \hat{\bar{\sigma}}_{\dev,i,\dep}}
    \dif\bar{p}.
    \label{eq219}
\end{align}
%---------------------------------------------------------------------------------------
where $\hat{\ta u}^{\macro(i)}_\dev = \ts E_i \cdot[\ta x - \bar{\ta x}]$.
...
\begin{alignat}{3}
    (a_\rve)'(\ta{u};\delta\ta{u}^\fluct, \hat{\ta u}_\ded^{\fluct(k)}) + b_\rve(\hat{p}_\ded^{(k)},\delta\ta{u}^\fluct) &= -(a_\rve)'(\ta{u};\delta\ta{u}^\fluct, \hat{\ta{u}}^{\macro(k)}_\dev)
    &\quad& \forall \delta\ta{u}^\fluct &&\in \set{U}_\rve^{\Dirichlet,0}
\\
    b_\rve(\delta p,\hat{\ta u}_\ded^{\fluct(k)} + \hat{\bar{e}}_{\ded}^{(k)}\,\ta{x}_\mean) + (c^*_\rve)'(p;\delta p,\hat{p}_\ded^{(k)}) &= 0
    &\quad& \forall \delta p &&\in \set{P}_\rve
\\
...
\begin{alignat}{3}
    (a_\rve)'(\ta{u}^\macro + \ta{u}^\fluct;\delta\ta{u}^\fluct, \hat{\ta u}_\dep^\fluct) + b_\rve(\hat{p}_\dep,\delta\ta{u}^\fluct) &= 0
    &\quad& \forall \delta\ta{u}^\fluct &&\in \set{U}_\rve^{\Dirichlet,0}
\\
...
% The volumetric strain, $\hat{\bar{e}}_{\ded}^{(k)}$ and $\hat{\bar{e}}_{\dep}$, is solved for directly, and the deviatoric stress is post-processed as
% \begin{align}
%  \hat{\bar{\ts\sigma}}_{\dev,\ded}^{(k)} &=  \frac{1}{\volume} \int_{\Gamma_\rve} \hat{\ta t}_{\ded}^{(k)} \outerp [\ta x-\bar{\ta x}]\dif S\\
%  \hat{\bar{\ts\sigma}}_{\dev,\dep} &= \frac{1}{\volume} \int_{\Gamma_\rve} \hat{\ta t}_{\dep} \outerp [\ta x-\bar{\ta x}]\dif S
% \end{align}
