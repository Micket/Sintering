...
We are now in the position to establish the subscale problem: For \emph{given} values $\bar{\ta{u}}$, $\bar{\ts{h}}\defeq\bar{\ta{u}}\outerp\diff$, and $\bar{p}$, that represent the macroscale fields (and which solve the macroscale problem), we first make the unique decomposition:
%----------------------------------------------------------------------------
...
The motivation for the decomposition of tractions in \cref{eq29c} is the observation whereby \cref{eq27a} can be restricted to test functions in $\Pi_\rve^\fluct$, such that the stress quantity $\bar{\ts\tau}$ is not part of the RVE-problem
%----------------------------------------------------------------------------
\begin{equation}
...
%----------------------------------------------------------------------------
where $\ta{x}_\mean\defeq\frac{1}{3}[\ta{x}-\bar{\ta{x}}]$.
It follows from \cref{eq51} that the variable $\bar{e}$ still obtains the value $\bar{e}\{\bar{\ts\epsilon}_\dev, \bar{p}\} \defeq \homgen{\hat{e}(p)}$
\todo{last step? wrong $\delta$}
\begin{align}
 \bar{e}\{\bar{\ts\epsilon}_\dev, \bar{p}\}\,\delta\bar{p} = \homgen{\hat{e}(p)}\,\delta\bar{p} = b_\rve(\delta\bar{p}, \ta u) = d_\rve(\delta\bar{p}\,\ta n, \ta u) = d_\rve(\delta\bar{p}\,\ta n, \bar{e}\,\ta x_\mean) = \delta\bar{p}\,\bar{e}
\end{align}
where the divergence theorem was used in the step $b_\rve(\delta\bar{p}, \ta u) = d_\rve(\delta\bar{p}\,\ta n, \ta u)$.
...
\begin{subequations}
\begin{align}
    \ta u^\fluct &= \ta u - \bar{\ts\epsilon}_\dev \cdot[\ta x - \bar{\ta x}] - \bar{e}\,\ta x_\mean
\label{eq52a} \\
    p^\fluct &= p - \bar{p}
\label{eq52b} \\
    \ta t^\fluct &= \ta t - [\bar{\ts\sigma}_\dev - \bar{p}\ts I]\cdot \ta n
\label{eq52c}
\end{align}
\end{subequations}
% \begin{align}
