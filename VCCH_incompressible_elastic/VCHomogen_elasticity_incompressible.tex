...
\todo{new}
As shown in appendix \cref{appendix:1}, the VCMC is satisfied is $R_\rve(z, (z^\fluct)'\{z^\macro;\delta z^\macro\}) = 0$. In this case, the VCMC is
...
\end{align}
\end{subequations}
We will also need to make use the tangent problem of \cref{eq31} at equilibrium with a perturbation of the macroscopic fields $\ta u^\macro$ and $p^\macro$ which yields
\begin{subequations}
\begin{alignat}{3}
...
\end{alignat}
\end{subequations}
where the sensitivity field $(\ta u^\fluct)'\{\ta u^\macro, p^\macro;\delta \ta u^\macro, \delta p^\macro\}$ is defined as the solutions $\dif\ta u^\fluct$.
...
\begin{subequations}\label{eq67}
\begin{alignat}{3}
    a_\rve(\ta{u};\delta\ta{u}) + b_\rve(p,\delta\ta{u}) +  d_\rve(\tilde{\bar{\ts\sigma}}_\dev\cdot,\delta\ta{u}) &= d_\rve(\bar{p}\,\ta n,\delta\ta u)
    & \quad & \forall \delta\ta{u} &&\in \set{U}_\rve
\label{eq67a} \\
    b_\rve(\delta p,\ta{u}) + c^*_\rve(p;\delta p) &= 0
    & \quad & \forall \delta p &&\in \set{P}_\rve
