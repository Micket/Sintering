...
The larger SVE:s do not need as many realizations, which is verified by the small variance as can be seen in \Cref{fig:SVE_comp}.
...
In \Cref{fig:RVE_compressibility} it is shown how the homogenized properties are affected by the matrix compliance, $C_\mathrm{mat}$, for a given (fixed) realization and given SVE-size ($L_\rve = 9$, cf. \Cref{fig:rve_sample9}),  thus defining an RVE.
