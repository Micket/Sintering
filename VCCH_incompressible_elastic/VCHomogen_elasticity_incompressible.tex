...
For reference on the classical approaches, selected literature by \cite{fish_practical_2013, torquato_random_2006, zohdi_introduction_2004} gives a good overview.

Among the work on extending the types of boundary conditions for the microscale problem, we mention 
\cite{coenen_multi-scale_2012,geers_multi-scale_2010,larsson_computational_2011,temizer_optimality_2013}, and work on interfaces and shells \cite{mcbride_micro--macro_2012, larsson_stress-resultant_2013}.
...
This process is thus characterized by a transition from the compressible to incompressible regimes that should be handled within the same variational framework, cf.\  \textsc{Öhman et al.\ (2013)} \cite{ohman_computational_2013}.
...
\subsection{Convergence of homogenized quantities}
...
\caption{A sample RVE subject to Neumann boundary conditions and macroscopic shear $\bar{\epsilon}_{\dev,ij} = 0.4(1 - \delta_{ij})$.}
\label{fig:def_rve6}
\end{figure}

The Statistical Volume Element (SVE) is constructed from a series of randomly generated RVEs containing on average 10\% spherical inclusions with a mean diameter of one length unit.
A sample RVE is shown in \cref{fig:initial_rve6}.
The finite element discretization consist of tetrahedral Mini elements.
...
\subsection{Seamless transition from compressible to incompressible response}
...
We have shown the bounds for the periodic boundary condition on an RVE by deriving the Dirichlet and Neumann boundary conditions.
...
The implementation of all three boundary conditions is available in the open source C++ code OOFEM (\texttt{www.oofem.org}) \cite{patzak_design_2001} under the name \texttt{MixedGradientPressure}.
