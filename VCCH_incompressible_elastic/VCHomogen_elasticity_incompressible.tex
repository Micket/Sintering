...
Results from varying the $C_\mathrm{mat}$ in a single RVE subject to  are shown in \cref{fig:RVE_compressibility}.
When $C_\mathrm{mat} = 0$, macroscopic incompressibility is obtained which is reflected by $\bar{C} = 0$.
We note that also the macroscale shear modulus $\bar{G}$ is effected.
The RVE microstructure for \cref{fig:RVE_compressibility} is shown in \cref{fig:RVE_sample}.
...
\centering
\begin{tikzpicture}
  \begin{axis}[ width=0.45\linewidth, height=0.3\linewidth, xlabel=$C_\mathrm{mat}\times G_\mathrm{mat}$, ylabel=$\bar{C}\times G_\mathrm{mat}$,
    extra x ticks={1.0,0.8, 0.5,0.3,0.1},
...
\end{tikzpicture}
\begin{tikzpicture}
  \begin{axis}[ width=0.45\linewidth, height=0.3\linewidth, xlabel=$C_\mathrm{mat}\times G_\mathrm{mat}$, ylabel=$\bar{G}/G_\mathrm{mat}$]
  \addplot table[color=blue,x=M,y=G] {macro_k9.matdata};
  \end{axis}
\end{tikzpicture}
\caption{Homogenized results from a single RVE subject to Dirichlet boundary condition and varying the matrix material compressibility ($C_\mathrm{mat}$).}
...
\caption{RVE with incompressible inclusions in a compressible matrix.}
