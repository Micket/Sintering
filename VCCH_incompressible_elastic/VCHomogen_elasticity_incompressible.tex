...
\begin{align}
    ...
    \frac{1}{\volume} \int_{\Omega_\rve} \ta{u}\outerp\diff \dif V = \bar{\ta{h}}
\label{eq14a} \\
    \frac{1}{\volume} \int_{\Omega_\rve} p \dif V = \bar{p}
\label{eq14b}
\end{align}
...
\label{eq28b}
\end{align}
by introducing the constraints in \cref{eq13a,eq13b}.
...
To recap; for \emph{given} values $\bar{\ta{u}}$, $\bar{\ts{h}}$, and $\bar{p}$, that represent the macroscale fields (and which solve the macroscale problem), the subscale fields are decomposed as:
%----------------------------------------------------------------------------
...
We are now in the position to establish the subscale problem: Finding the subscale fluctuations $(\ta{u}^\fluct,p^\fluct,\ta{t}^\fluct)\in\set{U}_\rve^\fluct\times\set{P}_\rve^\fluct\times\set{T}_\rve^\fluct$ that solve the system
%----------------------------------------------------------------------------
...
As shown in appendix \cref{appendix:1}, the VCMC is satisfied if $R_\rve(z, (z^\fluct)'\{z^\macro;\delta z^\macro\}) = 0$. In this case, the VCMC is
...
This can be shown by introducing the unique split of the unknowns
\begin{align}
...
\end{align}
and inserting them into \cref{eq51c} and testing with $\delta\check{\ts\tau}$ we obtain
\begin{gather}
...
\begin{alignat}{3}
    ...
    &\quad& \forall \delta\bar{e} &&\in \set{R}
\end{alignat}
which implies that $\bar{p} = -\frac13 \check{\ts\tau}\dprod\ts I = \check{p}$.
Using these identities and testing \cref{eq51a,eq51b,eq51c} with the fluctuations $\delta\ta u^\fluct$, $\delta p^\fluct$, and $\delta\ta t^\fluct$ respectively, we obtain exactly the original system in \cref{eq31}.
