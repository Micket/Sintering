...
An important example is the evolving porous microstructure of a PM-product during the process of sintering, whereby the homogenized response is compressible until the porosity vanishes inferring incompressible macroscale response.
...
in terms of the macroscale solution $\bar{\ta{u}}(\bar{\ta{x}})$ and $\bar{p}(\bar{x})$ respectively.
...
\end{align}
% C1:
Utilizing this split, we use the fact that 
\begin{align}
d_\rve(\ta t,\delta\ta u^\fluct) = d_\rve(\ta t^\fluct, \delta\ta u^\fluct)\quad \forall\delta\ta u^\fluct \in\set{U}_\rve^\fluct
\end{align}
following from \cref{eq:dtauu0}.
% C2:
The proof of the uniqueness of \cref{eq28c} follows from considering 
\begin{align}
...
\end{align}
Hence, $\ta t - \bar{\ts\tau}[\ta t]\cdot\ta n \in\set{T}_\rve^\fluct$ which concludes the proof.
...
Finally, adoption \cref{eq33a,eq33b}, the canonical form results in the identical responses $\bar{\ts\sigma}_\dev\epspargs$ and $\bar{e}\epspargs$ as the original form presented in \Cref{sec:original_rve}.
...
We now define the volume-specific ``macroscale energy density''\footnote{$\bar{\psi}_\rve \to \psi_\rve$, the ``effective energy density'' for sufficiently large RVE. } as the value of $\Pi_\rve$ at the following generalized saddle-point:
%----------------------------------------------------------------------------------------------------------------
\begin{equation}
...
For the first numerical example, we have chosen a incompressible matrix with incompressible spherical inclusions.
...
From the numerical simulations, we compute the homogenized macroscale shear modulus defined as
\begin{align}
...
In the second example, we show how the homogenization scheme can seamlessly handle both compressible and incompressible materials.
Here, incompressible spherical inclusions, defined by $\hat{\ts\sigma}_\dev(\ts\epsilon_\dev) = 10 G_\mathrm{mat}\ts\epsilon_\dev$ and $\hat{e}(p) = 0$, are embedded in a matrix material defined by $\hat{\ts\sigma}_\dev(\ts\epsilon_\dev) = 2 G_\mathrm{mat}\ts\epsilon_\dev$ and $\hat{e}(p) = - C_\mathrm{mat}\,p$.
Results from varying the $C_\mathrm{mat}$ in a single RVE subject to Dirichlet boundary condition are shown in \cref{fig:RVE_compressibility}.
Poisson's ratio can be computed from the parameters $G$ (shear elasticity modulus) and $C$ (bulk compliance modulus) according to
\begin{align}
 \nu = \frac{3-2 C G}{6 + 2 C G}
\end{align}
where selected values have been been marked in \cref{fig:RVE_compressibility}.
With $C_\mathrm{mat} = 0$ macroscopic incompressibility is also obtained (defined by $\bar{C} = 0$).
We note that also the macroscale shear modulus $\bar{G}$ is effected.
The microstructure for the RVE in \cref{fig:RVE_compressibility} is shown in \cref{fig:RVE_sample}.

The homogenization scheme seamlessly handles the transition from $C_\mathrm{mat} = 0$ to $C_\mathrm{mat} > 0$ without having to perform any other changes than the material parameter.


\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[ width=0.45\linewidth, height=0.3\linewidth,
...
\begin{tikzpicture}
  ...
  \node at (2.8,4.4) { $\nu_\mathrm{mat}$ };
\end{tikzpicture}
\caption{Homogenized results from a single RVE subject to Dirichlet boundary condition and varying the matrix material compressibility ($C_\mathrm{mat}$) while keeping all other parameters fixed.}
\label{fig:RVE_compressibility}
\end{figure}

\begin{figure}[H]
\centering
 \includegraphics[width=0.45\linewidth]{rve9.png}
 \includegraphics[width=0.45\linewidth]{rve9_inc.png}
\caption{RVE with dimensions $9\times9\times9$ with incompressible inclusions in a (in)compressible matrix.}
\label{fig:RVE_sample}
\end{figure}
...
Due to the difficult procedure of constructing basis functions for $\set T_\rve$ for 3D geometries, the current implementation relies on global polynomials where the computational cost drastically increased with the order of the polynomials while only showing a marginal improvement compared to the Neumann boundary condition.
As a possible alternative, it is straight forward to derive a strongly periodic boundary condition by replacing \cref{eq61b} with
\begin{align}
    \set{U}_\rve^{\Periodic,\fluct} &= \{\ta{v}\in \set{U}_\rve^\fluct \,|\,\, \jmp{\ta{v}}=\ta{0} \text{ on } \Gamma_\rve \}
\end{align}
and performing the steps following up to \cref{eq:weak_form_dirichlet}.
While such implemented wouldn't need costly Lagrange multipliers for $\set{T}_\rve$, it instead requires a meshing software capable of creating periodic meshes.

The implementation of all three boundary conditions is available in the open source C++ code OOFEM (\texttt{www.oofem.org}) under the name \texttt{MixedGradientPressure}.
