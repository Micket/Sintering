...
This choice corresponds to the standard relation
...
The target volume fraction of particles if 10\% and their size-distribution, in terms of diameter, is uniform distribution in the range $[\frac23 \;\;\frac43]$ with has a mean value of $1$.
Two examples of realizations with SVE-size $L_\rve = 6$ and $L_\rve = 9$ are shown in \cref{fig:rve_sample6,fig:rve_sample9} respectively.
...
However, even for a smaller SVE-size, it is possible to compute an ``apparent'' value of $\bar{G}$ by the projection
\begin{align}
 \bar{G} = \frac12 \frac{\sum_{ijkl}(\tf I_\dev)_{ijkl}\bar{\tf E}_{ijkl}}{\sum_{ijkl} (\tf I_\dev)_{ijkl}^2}
\end{align}
where $\tf I_\dev$ is the deviatoric fourth order identity tensor such that the isotropic part of $\bar{\tf E}$ could be expressed as $\bar{\tf E}_{\mathrm{iso}} = 2 \bar{G} \tf I_\dev$ in standard fashion.
...
This type of element is defined by linear approximations plus bubble function for $\ta u$ and linear approximation for $p$, cf.\ \cite{arnold_stable_1984}.
...
Convergence results for the two extremes of Dirichlet and Neumann boundary conditions are shown in \cref{fig:SVE_comp}.
...
Here, a fixed number of realizations are used for each value of $L_\rve$, starting with 200 realizations for the smallest SVE size ($L_\rve = 1$) and gradually decreases to only 4 realizations for the largest SVE-size ($L_\rve$). The larger SVE:s do not need as many realizations, since the variance is very small, as can be seen in \cref{fig:SVE_comp}.

\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[ width=0.45\linewidth, height=0.28\linewidth, xlabel=SVE size, ylabel=$\mathop{\mathrm{mean}}(\bar{G})/G_\mathrm{mat}$]
...
\end{tikzpicture}
\begin{tikzpicture}
  \begin{axis}[ width=0.45\linewidth, height=0.28\linewidth, xlabel=SVE size, ylabel=$\mathop{\mathrm{var}}(\bar{G})/G_\mathrm{mat}$]
...
The deformed shape for a typical sample SVE of size $L_\rve = 6$ with Neumann boundary condition is shown in \cref{fig:def_rve6}

\begin{figure}[htpb!]
\centering
 \includegraphics[width=0.5\linewidth]{rve6_def.png}
\caption{A sample RVE subjected to macroscopic pure shear $(\bar{\ts\epsilon}_\dev)_{ij} = 0.4[1 - \delta_{ij}]$.}
\label{fig:def_rve6}
\end{figure}
...
In \cref{fig:RVE_compressibility} it is shown how the homogenized properties are affected by the matrix compliance, $C_\mathrm{mat}$, for a given (fixed) realization and given SVE-size ($L_\rve = 9$, cf. \cref{fig:rve_sample9}).
...
\end{align}
which is also used in \cref{fig:RVE_compressibility}.
...
Dependence of effective properties $\bar{C}$ and $\bar{G}$ on the matrix-bulk compliance $C_\mathrm{mat}$ for fixed values of $G_\mathrm{part} = 5\,G_\mathrm{mat}$ and $C_\mathrm{part} = 0$.}
...
We have derived a canonical form of the RVE-problem which has the same strong form as the macroscale problem which still fulfills the macrohomogeneity condition.
