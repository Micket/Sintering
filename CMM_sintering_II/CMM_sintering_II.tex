...
\begin{align}
  \bar{\sigma}_\dev &= \pd{\bar\Pi}{\bar{\ts d}_\dev} = \pd{\Pi_\Box}{\bar{\ts d}_\dev}\\
...
\begin{align}
 \Psi_\Box'(\ta v;\delta\ta v) = a_\Box(\ta v;\delta\ta v)
\end{align}
and abbreviate $\ta v = \bar{\ts d}_\dev\cdot[\ta x-\bar{\ta x}] + \bar d_\vol \ta x_\mean + \ta v^\fluct$.
We have directly that \eqref{eq:potential_rve_problem} is equivalent to \eqref{eq213}.
Furthermore,
\begin{alignat}{3}
 \pd{\bar\Pi}{\bar{\ts d}_\dev} &= \pd{\Pi_\Box}{\bar{\ts d}_\dev} &&= \sum_i \left[ a_\Box(\ta v;\ts E_i\cdot[\ta x-\bar{\ta x}])-l_\Box^\pore(\ts E_i\cdot[\ta x-\bar{\ta x}]) \right]\ts E_i = \bar{\ts\sigma}_{\dev,\Box}\\
 \pd{\bar\Pi}{\bar{p}} &= \pd{\Pi_\Box}{\bar{p}} &&= \bar d_\vol = \bar d_{\vol,\Box}
\end{alignat}
which is identical to the definition of $\bar{\ts\sigma}_{\dev,\Box}\{\bar{\ts d}_\dev,\bar p\}$ and $\bar{d}_{\vol,\Box}\{\bar{\ts d}_\dev,\bar p\}$.
