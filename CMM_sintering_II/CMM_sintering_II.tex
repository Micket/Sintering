...
\begin{figure}[thpb!]
  ...
  \begin{axis}[ yticklabel style={ /pgf/number format/fixed, /pgf/number format/precision=2 },
                ...
                height=0.2\textheight, width=0.9\textwidth, xmin=0, xmax=1, ymin=0.80,
                legend style={font=\tiny,at={(1,0)},anchor=south east},
                xlabel={$t_{\mathrm{end}}$}, ylabel={$\Phi$}]
    \addplot[black] table[x=t,y=free] {figures/evolve_gp.txt};
    \addlegendentry {$\bar{\ts\sigma}_\dev = \ts 0, \bar{p} = 0$}
    \addplot[red] table[x=t,y=shear] {figures/evolve_gp.txt};
...
Another unit cell was subjected to zero macroscopic stress, i.e. $\bar{\ts\sigma} = \ts 0$. Snapshots of the evolving RVE-configuration are given in \figref{fig:evolve_free}. The evolution of the relative density $\Phi$ is shown in \figref{fig:evolve_graph}.
