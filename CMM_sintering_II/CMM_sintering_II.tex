...
\subsection{Extension to macroscale incompressibility}
In the previous paper, \cite{Ohman2011a}, only the momentum balance equation was homogenized, such that the macroscopic rate of deformation gradient, $\bar{\ts d}$,
was prolonged to the RVE problem, and the corresponding macroscopic stress tensor $\bar{\ts\sigma}$ was obtained through homogenization, i.e.
\begin{align}
 \bar{\ts d} \longrightarrow \bar{\ts\sigma}
\end{align}
or alternatively, by splitting into volumetric and deviatoric parts
\begin{alignat}{2}
 &\bar{\ts\sigma}\to \bar{\ts\sigma}_\dev, \bar{p}&\quad&\text{where}\quad \bar{p} \defeq -\frac13 \bar{\ts\sigma}\dprod \ts I,\quad \bar{\ts\sigma}_\dev \defeq \bar{\ts\sigma} - \bar{p} \ts I \\ 
 &\bar{\ts d}\to \bar{\ts d}_\dev, \bar{d}_\vol   &\quad&\text{where}\quad \bar{d}_\vol \defeq \bar{\ts d}\dprod \ts I,\quad \bar{\ts d}_\dev \defeq \bar{\ts d} - \frac13\bar{d}_\vol\ts I
\end{alignat}
we can write an equivalent homogenization procedure as
\begin{align}
 \label{eq:old_homogenization} \bar{\ts d}_\dev, \bar{d}_\vol \longrightarrow \bar{\ts\sigma}_\dev, \bar{p}.
\end{align}
We also note that the relation between the rate of deformation gradient and velocity
\begin{align}
 \bar{\ts d}_\dev &= [\bar{\ta v} \outerp\diff]^\sym_\dev \\
\label{eq:dbarvol} \bar{d}_\vol &= \bar{\ta v}\cdot\diff
\end{align}

The macroscale problem consist only of momentum balance equation; find $\bar{\ta v} \in \bar{\set{V}}$ such that
\begin{align}
 \label{eq:old_macro} \int_\Omega \bar{\ts\sigma}\{\bar{\ts d}\} \dprod [\delta\bar{\ta v}\outerp \diff] \dif v = 0 \quad \forall \delta\bar{\ta v}\in \bar{\set{V}}^0.
\end{align}
while on each RVE, the incompressible Stokes' Flow problem is as follows; find $(\ta v^\fluct,p) \in \set{V}_\Box\times\set{P}_\Box$ such that
\begin{subequations}\label{eq:old_rve}
\begin{alignat}{2}
\label{eq:old_rve_momentum}
 &\frac{1}{|\Omega_\Box|}\int_{\Omega^\particle_\Box} \ts{\sigma}\dprod[\delta\ta v^\fluct\outerp\diff]^\sym \dif v
   = \frac{1}{|\Omega_\Box|}\int_{\Gamma^\pore_\Box} \ta{t}_\surf \cdot \delta\ta v^\fluct \dif a &\quad& \forall \delta\ta v^\fluct\in\set{V}^0_\Box\\
\label{eq:old_rve_cont}
 &\frac{1}{|\Omega_\Box|}\int_{\Omega^\particle_\Box} \left[\ta{v}\cdot\diff\right] \delta p \dif v
   =  0 &\quad& \forall \delta p\in\set{P}_\Box
\end{alignat}
\end{subequations}
for the prolonged velocity $\ta v = \bar{\ta d}\cdot[\ta x-\bar{\ta x}] + \ta v^\fluct$.
Considering \eqref{eq:old_rve_momentum}, $\ts\sigma = \ts\sigma_\dev(\ts d) - p \ts I$, where $\ts d\defeq[\ta v\outerp\diff]^\sym$ and $p$ is the Lagrange multiplier pertinent to the condition in \eqref{eq:old_rve_cont}.
The macroscopic stress, $\bar{\ts\sigma}\{\bar{\ts d}\}$, is obtained through post-processing.

This procedure breaks down when the porosity vanishes.
However, this can be countered by relaxing \eqref{eq:dbarvol} through a Lagrange multiplier for the macroscopic pressure.
For the RVE problem, this means a mixed control is introduced, whereby the homogenization procedure in \eqref{eq:old_homogenization} is exchanged for
\begin{align}
 \label{eq:new_homogenization} \bar{\ts d}_\dev, \bar{p} \longrightarrow \bar{\ts\sigma}_\dev, \bar{d}_\vol.
\end{align}
and the the macroscale problem in \eqref{eq:old_macro} is reformulated to find $(\bar{\ta v},\bar{p}) \in \bar{\set{V}}\times\bar{\set{P}}$ such that
...
\begin{alignat}{2}
 ...
 &\int_\Omega \left[\bar{d}_\vol\{\bar{\ts d}_\dev,\bar{p}\} - \bar{\ta v}\cdot\diff\right]\delta\bar{p} \dif v &\quad& \forall \delta\bar{p}\in \bar{\set{P}}
\end{alignat}
\end{subequations}
As before, the prolongation of $\bar{\ts d}$ on the RVE remains and can be expanded as
\begin{align}
\ta v = \bar{\ts d}_\dev\cdot [\ta x-\bar{\ta x}]+ \bar{d}_\vol \frac13 [\ta x - \bar{\ta x}] + \ta v^{\fluct}
\end{align}
but as its volumetric part is now an unknown output from the RVE-problem.
Testing the subscale momentum balance with $\delta\bar{d}_\vol \frac13[\ta x-\bar{\ta x}]$ yields a third equation for the RVE problem (steps are detailed in Appendix A1).
We then obtain the following RVE-problem; find $(\ta v^\fluct,p,\bar{d}_\vol)\in \set{V}_\Box \times \set{P}_\Box \times \set{R}$ such that
\begin{subequations}\label{eq:new_rve}
\begin{alignat}{2}
 &\frac{1}{|\Omega_\Box|}\int_{\Omega^\particle_\Box} \left[\ts{\sigma}_\dev\dprod[\delta\ta v^\fluct\outerp\diff] - p\;[\delta\ta v^\fluct \cdot \diff] \right]\dif v
   = \frac{1}{|\Omega_\Box|}\int_{\Gamma^\pore_\Box} \ta{t}_\surf \cdot \delta\ta v^\fluct \dif a &\quad& \forall \delta\ta v^\fluct\in\set{V}^0_\Box\\
 &\frac{1}{|\Omega_\Box|}\int_{\Omega^\particle_\Box} \left[\ta{v}\cdot\diff\right] \delta p \dif v
   =  0 &\quad& \forall \delta p\in\set{P}_\Box \\
 &\frac{1}{|\Omega_\Box|}\int_{\Omega^\particle_\Box} p\dif v \;\delta\bar{d}_\vol 
   =  \left[\frac{1}{|\Omega_\Box|}\int_{\Gamma^\pore_\Box} \frac13 \ta{t}_\surf \cdot [\ta x-\bar{\ta x}] \dif a -\bar p\right]\delta\bar{d}_\vol &\quad& \forall \delta\bar{d}_\vol\in\set{R}
\end{alignat}
\end{subequations}
From which the macroscale properties used in \eqref{eq:new_macro} is obtained; $\bar{\ts\sigma}_\dev\{\bar{\ts d}_\dev,\bar p\}$ is post-processed while $\bar{d}_\vol$ is solved for directly.

\subsubsection{Summary of old FE\textsuperscript{2} procedure}
\begin{itemize}
 \item Macroscale-problem
 \begin{itemize}
  \item Fields: $\bar{\ta v}$, solved from \eqref{eq:old_macro}
 \end{itemize}
 \item RVE-problem
 \begin{itemize}
  \item Input: $\bar{\ts d}$
  \item Fields: $\ta v^\fluct$, $p$, solved from \eqref{eq:old_rve}
  \item Output: $\bar{\ts\sigma}$ (post-processed)
 \end{itemize}
\end{itemize}

\subsubsection{Summary of new FE\textsuperscript{2} procedure}
\begin{itemize}
 \item Macroscale-problem
 \begin{itemize}
 ...
 \begin{itemize}
  ...
  \item Fields: $\ta v^\fluct$, $\bar{d}_\vol$, $p$, solved from \eqref{eq:new_rve}
  \item Output: $\bar{\ts\sigma}_\dev$ (post-processed), $\bar{d}_\vol$
 \end{itemize}
\end{itemize}
...
\begin{align}
 \bar{c}(\ta v^\fluct, \delta p) &= \int_{\Omega} \underbrace{\frac{1}{|\Omega_\Box^\particle|} \int_{\Omega^\particle_\Box}[\tilde{\ta v}^\fluct\cdot\diff]\dif v}_{\bar{d}^\fluct_{\vol}} \dif v.
\end{align}
The RVE problem is thus composed by $\tilde v^\fluct\{\ta v^\macro,p^\macro\}$, $\tilde p^\fluct\{\ta v^\macro,p^\macro\}$, and $\bar d_{\vol}^\fluct\{\ta v^\macro,p^\macro\}$.
...
We also replace replace the standard dyad bases with a complete, orthonormal, basis $\{\ts E_i\}_{i=1}^{n_{\mathrm{b}}}$ for symmetric deviatoric tensors.
