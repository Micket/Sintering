...
\end{align}
On each RVE, the incompressible Stokes' Flow problem is solved
\begin{subequations}\label{eq:old_rve}
\begin{alignat}{2}
 &\int_{\Omega^\particle_\Box} \ts{\sigma}\dprod[\delta\ta v^\fluct\outerp\diff]^\sym \dif v
   = \int_{\Gamma^\pore_\Box} \ta{t}_\surf \cdot \delta\ta v^\fluct \dif a &\quad& \forall \delta\ta v^\fluct\in\set{V}^0_\Box\\
 &\int_{\Omega^\particle_\Box} \left[\ta{v}\cdot\diff\right] \delta p \dif v
   =  0 &\quad& \forall \delta p\in\set{P}_\Box
\end{alignat}
\end{subequations}
for the prolonged velocity $\ta v = \bar{\ta d}\cdot[\ta x-\bar{\ta x}] + \ta v^\fluct$.
The macroscale however only consist only of momentum balance equation
\begin{align}
 \label{old:macroscale} \int_\Omega \bar{\ts\sigma}\{\bar{\ts d}\} \dprod [\delta\bar{\ta v}\outerp \diff]^\sym \dif v = 0 \quad \forall \delta\bar{\ta v}\in \bar{\set{V}}^0.
\end{align}

This procedure breaks down when the porosity vanishes.
However, this can be countered by softening \eqref{eq:dbarvol} through a Lagrange multiplier for the macroscopic pressure.
...
\end{align}
and the the macroscale problem in \eqref{old:macroscale} becomes
\begin{subequations}\label{eq:new_macro}
\begin{alignat}{2}
 &\int_\Omega \bar{\ts\sigma}_\dev\{\bar{\ts d}_\dev,\bar{p}\} \dprod [\delta\bar{\ta v}\outerp \diff]^\sym \dif v + \int_\Omega -\bar{p}\;[\delta\bar{\ta v}\cdot\diff]\dif v = 0 &\quad& \forall \delta\bar{\ta v}\in \bar{\set{V}}^0\\
...
\end{alignat}
\end{subequations}
As before, the prolongation of $\bar{\ts d}$ on the RVE remains, but as its volumetric part is now unknown, 
\begin{align}
\ta v = \bar{\ts d}_\dev\cdot [x-\bar{x}]+ \bar{d}_\vol \frac13 [\ta x - \bar{\ta x}] + \ta v^{\fluct}
\end{align}
we test with $\delta\bar{d}_\vol \frac13[\ta x-\bar{\ta x}]$, which yields a third equation for the RVE problem, and we obtain
...
\begin{alignat}{2}
 ...
 &\int_{\Omega^\particle_\Box} \left[\ta{v}\cdot\diff\right] \delta p \dif v
   =  0 &\quad& \forall \delta p\in\set{P}_\Box \\
 &\int_{\Omega^\particle_\Box} \frac13 p\;[\ta{x}-\bar{\ta x}]\dif v \;\delta\bar{d}_\vol 
   =  \left[ \int_{\Gamma^\pore_\Box} \frac13 \ta{t}_\surf \cdot [\ta x-\bar{\ta x}] \dif a -\bar p\right]\delta\bar{d}_\vol &\quad& \forall \delta\bar{d}_\vol\in\set{R}
\end{alignat}
\end{subequations}

\subsection{Macro- and subscale coupling --- Variational multiscale setting}
One can also obtain an equivalent set of equations as \label{eq:new_rve} and \eqref{eq:new_macro} by directly homogenizing both the momentum balance and continuity equation in a variationally consistent way.
