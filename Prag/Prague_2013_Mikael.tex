...
\frametitle{RVE-problem for mixed control: Neumann b.c.}
 \begin{itemize}
  ...
  \item For given macroscale variables $\bar{\ts d}_\dev$ and $\bar p$, find ($\ta{v}^\fluct,p,\bar{\ts\sigma}_\dev)\in\times\set{V}_\Box^{\Dirichlet}\times\set{P}_\Box\times\set{R}_\dev^{3\times3}$ that solve the system
 \end{itemize}
\vspace{-2truemm}
\begin{alignat*}{4}
    & a_\Box(\ta{v};\delta \ta{v}^\fluct) +  b_\Box(p,\delta\ta{v}^\fluct)
