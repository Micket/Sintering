...
\label{eq3c} \\
    l_\surf(\ta{v}) &\defeq -\int_{\Gamma^\surf} \gamma_\surf \hat{\ta I} \dprod [\ta{v}\outerp\diff] \dif S 
\label{eq3d} \\
...
%----------------------------------------------------------------------------------------------------------------
\begin{align}
    a(\ta{v};\ta{w}) &\approx \int_\Omega [a_\rve(\ta{v};\ta{w}) - l_{\surf\rve\dev}(\ta w)] \dif V
\label{eq7a} \\
    b(q,\ta{v}) &\approx \int_\Omega [b_\rve(q,\ta{w}) - l_{\surf\rve\vol}(\ta w)] \dif V
\label{eq7b} \\
    c^*(q;r) &\approx \int_\Omega c^*_\rve(q;r) \dif V
\label{eq7c} \\
    l(\ta{v}) &\approx \int_\Omega l_\rve(\ta{v}) \dif V + \int_{\Gamma^\Neumann} l_\#(\ta{v}) \dif S
\label{eq7d} \\
    l_\surf(\ta{v}) &\approx 0
\label{eq7e}
\end{align}
%---------------------------------------------------------------------------------------------------------------------
...
\label{eq8b} \\
    ...
    \homgen{ r\,\hat{e}(q) }
\label{eq8c} \\
    l_{\surf\rve\dev}(\ta{v}) &\defeq
    \frac{1}{\volume} \int_{\Gamma_\rve^\pore} \gamma_\surf \hat{\ts I} \dprod [\ta{v}\outerp\diff]_\dev \dif S
\label{eq8d} \\
    l_{\surf\rve\vol}(\ta{v}) &\defeq
    \frac{1}{\volume} \int_{\Gamma_\rve^\pore} \gamma_\surf \hat{\ts I} \dprod [\ta{v}\outerp\diff]_\vol \dif S
\label{eq8e} \\
    l_\rve(\ta{v}) &\defeq
...
\label{eq14a} \\
    \frac{1}{\volume} \int_{\Gamma_\rve} \ta{t} \cdot [\ta x-\bar{\ta x}] \dif S = \bar{p}
\label{eq14b}
\end{gather}
\end{subequations}
...
\label{eq14a} \\
    \frac{1}{\volume} \int_{\Gamma_\rve} \ta{t}^\fluct \cdot [\ta x-\bar{\ta x}] \dif S = 0
\label{eq14b}
\end{gather}
\end{subequations}
%----------------------------------------------------------------------------
