...
% \todo{TODO}
% In this work, we aim to combine subscale structure, incompressible medium with pores from [paper2] with the VCH procedure laid out in \cite{ohman_variationally_2014}.
...
% For simplicity and generality, we will however the equivalent problem of elasticity, cf.\ \cite{ohman_variationally_2014}, throughout the derivations.,
% \todo{END}
...
\end{align}
%----------------------------------------------------------------------------
Note in particular the treatment of the surface tension in \eqref{eq7e}, which exists only in the RVE-problem, and which influences the macroscale problem only through \eqref{eq7a} and \eqref{eq7b}.
...
\begin{align}
    \ta v^\macro(\bar{\ta{x}};\ta{x}) &= \bar{\ta v}(\bar{\ta{x}}) + \bar{\ts{h}}(\bar{\ta{x}}) \cdot [\ta{x}-\bar{\ta{x}}], \quad \ta{x}\in\Omega_\rve^\particle
\label{eq12a} \\
    p^\macro(\bar{\ta{x}};\ta{x}) &= \densinv [\bar{p}(\bar{\ta{x}}) + \frac23 \shomgen{\gamma_\surf}], \quad^\particle
    \ta{x}\in\Omega_\rve
\label{eq12b}
\end{align}
\end{subequations}
where
\begin{align}
  \bar{\ts{h}}\defeq\bar{\ta v}\outerp\diff
\end{align}
%----------------------------------------------------------------------------
...
% TODO: ingen skippa härledningarna
...
As shown in \cite{ohman_variationally_2014}, we may introduce the unique split
\begin{align}
...
The Variationally Consistent Macrohomogeneity Condition (VCMC) (or generalized Hill-Mandel condition) is reviewed in \cite{ohman_variationally_2014}.
%With exception of the addition of $l_\rve^\pore(\bullet)$, in \cref{eq31} 
...
The Dirichlet boundary condition provides an upper bound for the periodic solution, i.e.\ $ \bar{\psi}_\rve^\Dirichlet \leq \bar{\psi}_\rve$.
By restricting the space $\set{V}_\rve$ such that the fluctuations $\ta v^\fluct = 0$ on $\Gamma_\rve$ we obtain the following problem:
...
\label{eq64b} \\
    ...
    l_\rve^\pore(\delta\bar{e}\,\ta x_\mean) - \bar{p}\,\delta\bar{e}
    &\quad& \forall \delta\bar{e} &&\in \set{R}
\label{eq64c}
\end{alignat}
\end{subequations}
...
The Neumann  boundary condition provides a lower bound for the periodic solution, i.e. $ \bar{\psi}_\rve \leq \bar{\psi}_\rve^\Neumann$.
...
The volumetric strain rate $\bar{e}$ is obtained by post-processing:
\begin{align}
...
\begin{table}[htbp!]
 \centering
% # IST_DeviatoricStress, IST_Pressure
% dirichlet 6 0.149130 -0.0750687 -0.0740610 0.0000111066 0.000312641 0.0000527840   1 -6.96048 
% periodic  6 0.165253 -0.0832048 -0.0820482 0.0000103426 0.000428893 0.0000733812   1 -6.98467 
