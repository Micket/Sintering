...
\item The additional surface-tension load $l_\surf(\ta v)$ in \eqref{eq2a}
\end{itemize}
In the special case that $\Omega^\particle = \Omega$, and thus $\Gamma^\pore = \emptyset$, then we retain the exact problem as in \cite{2014_ohman_...}.
...
In the special case $\Omega_\particle = \Omega$, we again retain the same $p^\macro$, $\bar{e}$ as in \cite{ohman_computational_2014}.
