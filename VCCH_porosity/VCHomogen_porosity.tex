...
\begin{subequations}
\begin{align}
 \int_\Omega \underbrace{[\homgen{ \hat{\ts{\sigma}}_\dev(\ts{\epsilon}_\dev[\ta{u}]) - p \ts I } + \shomgen{ \gamma_\surf \hat{\ts I}}]}_{=\bar{\ts\sigma}} \dprod [\delta\bar{\ta{u}}\outerp\diff] \dif V = \ldots
...
\begin{align}
 [\homgen{ \hat{\ts{\sigma}}_\dev(\ts{\epsilon}_\dev[\ta{u}]) - p \ts I } + \shomgen{ \gamma_\surf \hat{\ts I}}] \dprod[\delta\bar{\ta{u}}\outerp\diff]
  = [\homgen{ \ts{\sigma} } + \shomgen{ \gamma_\surf \hat{\ts I}}]\dprod[\delta\bar{\ta{u}}\outerp\diff]
\nonumber\\
  = \frac{1}{\volume} \Big[ \int_{\Gamma_\rve \cup \Gamma_\rve^\pore } \ta t \outerp[\ta x-\bar{\ta x}] \dif S + \shomgen{ \gamma_\surf \hat{\ts I}} \Big]\dprod[\delta\bar{\ta{u}}\outerp\diff]
\nonumber\\
  = \frac{1}{\volume} \Big[\int_{\Gamma_\rve} \ta t \outerp[\ta x-\bar{\ta x}] \dif S + \int_{\Gamma_\rve^\pore }  [\ta t_\surf \outerp[\ta x-\bar{\ta x}] + \gamma_\surf \hat{\ts I} ]\dif S \Big]\dprod[\delta\bar{\ta{u}}\outerp\diff]
\nonumber\\
  = \frac{1}{\volume} \Big[\int_{\Gamma_\rve} \ta t \outerp[\ta x-\bar{\ta x}] \dif S + \int_{\Gamma_\rve^\pore }  [\gamma_\surf\hat{\ts I}\cdot\diff\outerp[\ta x-\bar{\ta x}] + \gamma_\surf \hat{\ts I} ]\dif S \Big]\dprod[\delta\bar{\ta{u}}\outerp\diff]
\nonumber\\
 ????????????? = \frac{1}{\volume} \Big[\int_{\Gamma_\rve} \ta t \outerp[\ta x-\bar{\ta x}] \dif S \Big]\dprod[\delta\bar{\ta{u}}\outerp\diff]
 \label{eq:derived_macro_a2}
\end{align}
\todo{??}
where we now can identify the suitable definitions
%----------------------------------------------------------------------------------------------------------------
\begin{align}
...
\begin{subequations}
\begin{align}
    a_\rve(\ta{u};\delta\ta{u}) + b_\rve(\delta\ta{u},p) - \frac{1}{\volume}\int_{\Gamma_\rve} \ta{t}\cdot\delta\ta{u} \dif S &= l_\rve(\delta\ta{u})
\label{eq19a} \\
