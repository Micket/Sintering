...
%\newcommand{\to}[1]{\;{}^1#1}
%\renewcommand{\to}[1]{{\mathbf{#1}}}
\renewcommand{\to}[1]{\text{\boldmath $#1$}} % Bold, cursive
\newcommand{\ts}[1]{\text{\boldmath $\rm #1$}} % Bold, straigt
\newcommand{\td}[1]{\text{\boldmath $\mathcal{#1}$}} % Bold, 
\newcommand{\tf}[1]{\text{\boldmath $\sf #1$}} % Bold, sans-serif
...
\begin{gather}
 \um F^\element_\linear = \gamma t \left[{\um N^\element}^\T\right]_{a}^{b}\cdot \to e_s =
  \gamma t L^{-1} \begin{bmatrix}x_2-x_1\\y_2-y_1\\x_1-x_2\\y_1-y_2\end{bmatrix}\\
 \um F^\element_\linear = \gamma t \begin{bmatrix}m_x\\m_y\end{bmatrix}
\end{gather}
...
\begin{align}
 \to x[\xi,\eta,\zeta] = \zeta\sum_i N_i[\xi,\eta]\to x_i
\end{align}
...
=&\iint_\omega(\ts I-\to n\otimes\to n):\left((\to w\otimes\to\nabla_\xi) \cdot\ts J^{-1}\right)J\intd\xi\intd\eta\big|_{\zeta=1}\\
