...
\begin{align}
 \Psi^\Dirichlet_\Box(\bar{\ts d}_\dev, \bar{p} ) \defeq
    ...
    \sup_{p\in \set P_\Box} \Lambda_\Box(\bar{\ts d}_\dev, \bar{p}; \bar{\ts d}_\dev \cdot [\ta x - \bar{\ta x}] + \bar{e} \ta x_\mean + \ta v^\fluct, p, \bar{\ts\sigma}_\dev)
\end{align}
where
\begin{align}
 \set{V}_\Box^0 &= \{ \ta v \in \set{V}_\Box \;:\; \ta v = \ta 0 \text{ on }\Gamma_\Box \} \\
...
\begin{alignat}{2}
 ...
 l_\Box(\delta\ta v)        &\defeq -\volume \int_{\Gamma_\Box^\pore}\hat{\ts\sigma}\dprod[\delta\ta v \outerp\hat{\diff}] \dif A
\end{alignat}
\todo{what else should be mentioned}
% macroscopic response
% \begin{align}
%  \bar{\ts\sigma}_\dev &= \pd{\Psi_\Box^\Dirichlet}{\bar{\ts d}_\dev} \\
%  \bar{e} &= \pd{\Psi_\Box^\Dirichlet}{\bar{p}}
% \end{align}
